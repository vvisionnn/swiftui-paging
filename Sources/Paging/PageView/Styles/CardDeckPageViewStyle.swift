...
struct CardDeckPageView: View {
	...
	var dragGesture: some Gesture {
		DragGesture(minimumDistance: 5)
			.onChanged { value in
				dragProgress = -(value.translation.width / containerSize.width)
			}
			.onEnded { value in
				snapToNearestIndex()
			}
	...
	var swingOutMultiplier: Double {
		abs(sin(Double.pi * progressIndex) * 20)
	}
...
extension View {
	...
	public func pageViewCardCornerRadius(_ radius: Double?) -> some View {
		environment(\.cardCornerRadius, radius)
	}
	...
	public func pageViewCardShadow(_ visibility: Visibility) -> some View {
		environment(\.cardShadowDisabled, visibility == .hidden)
	}
